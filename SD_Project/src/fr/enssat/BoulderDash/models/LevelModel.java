...
public class LevelModel extends Observable implements Runnable {
    ...
    private RockfordModel rockford[] = {null, null};
    private GameInformationModel gameInformationModel;
    private int rockfordPositionX[] = {0, 0}, rockfordPositionY[] = {0, 0};
     ...
     */
    private void initRockford(int player) {
        this.rockfordPositionX[player] = this.levelLoadHelper.getRockfordPositionX(player);
        this.rockfordPositionY[player] = this.levelLoadHelper.getRockfordPositionY(player);
        this.rockford[player] = this.levelLoadHelper.getRockfordInstance(player);
    }
     ...
     */
    public void updateRockfordPosition(int player, int posX, int posY) {
        this.rockfordPositionY[player] = posY;
        this.rockfordPositionX[player] = posX;
    }
     ...
     */
    private void playCollisionSound(int player, int posX, int posY) {
        String collisionSound = null;

        if (this.getRockford(player).isCollisionDone() == false) {
            // Out of bounds?
            ...
            this.getRockford(player).setCollisionDone(true);
        }
     ...
     */
    public void setPositionOfRockford(int player, int posX, int posY) {
        int oldX = this.getRockfordPositionX(player);
        int oldY = this.getRockfordPositionY(player);
        ...
        this.playCollisionSound(player, posX, posY);
        ...
        if (this.isOutOfBounds(posX, posY) == false) {
            ...
            this.updateRockfordPosition(player, posX, posY);

            this.groundGrid[posX][posY] = this.getRockford(player);
        }
    }
     ...
     */
    public RockfordModel getRockford(int player) {
        return this.rockford[player];
    }
     ...
     */
    public void exploseGround(int player, int x, int y) {
        ...
        this.groundGrid[x + 1][y - 1] = new EmptyModel();
        this.groundGrid[x - 1][y - 1] = new EmptyModel();
        this.rockford[player].setHasExplosed(true);
